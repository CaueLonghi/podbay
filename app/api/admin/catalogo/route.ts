...
export async function POST(req: NextRequest) {
  ...
  const [result] = await db.query<mysql.ResultSetHeader>(
    `INSERT INTO catalogo (marca, sabor, descricao, tamanho, valor, estoque, emoji, categoria, ativo)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)`,
    [marca, sabor, descricao || null, tamanho, Number(valor), Number(estoque ?? 0), emoji || null, categoria || 'pods']
  );
